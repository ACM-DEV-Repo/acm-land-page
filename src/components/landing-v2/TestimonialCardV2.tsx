  ...
  return (
    <div className="glass-card relative p-8 pt-16 hover:scale-[1.02] transition-all duration-300">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
        {item.image ? (
          <img loading="lazy" src={item.image} className="w-20 h-20 rounded-full object-cover border-4 border-card shadow-lg" alt={item.name} />
        ) : (
      ...
      <div className="min-h-[220px] mt-4">
        <p className="italic text-base md:text-lg text-muted-foreground leading-relaxed text-center">"{item.text}"</p>
      </div>
