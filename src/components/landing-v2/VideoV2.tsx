...
type VideoV2Props = { data: LPContent["video"]; lpKey: string; couponCode?: string };
  ...
  return (
    <section className="w-full py-16 md:py-24 px-4 md:px-6">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="mb-12 md:mb-16 text-base md:text-lg lg:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">{data.subtitle}</p>
        <div className="w-full rounded-[var(--ds-radius)] aspect-video overflow-hidden glass-card flex items-center justify-center relative">
          {!embedUrl ? (
            ...
            <button onClick={() => setPlaying(true)} className="w-full h-full relative group cursor-pointer" aria-label="Reproduzir video">
              <img loading="lazy" src={thumbnail} alt={data.title || 'Video'} className="w-full h-full object-cover" />
            ...
            <iframe src={embedUrl} className="w-full h-full" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen title="Video" />
          )}
        </div>
