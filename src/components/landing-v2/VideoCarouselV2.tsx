...
type VideoCarouselV2Props = { data: LPContent["videoCarousel"]; lpKey: string; couponCode?: string };
    ...
    <CarouselItem className="pl-4 md:basis-1/2 lg:basis-1/3">
      <div className="glass-card rounded-2xl overflow-hidden shadow-lg border border-border/20 min-h-[250px]">
        <AspectRatio ratio={16 / 9}>
          {!playing && thumbnail ? (
            <button onClick={handlePlay} className="w-full h-full relative group cursor-pointer" aria-label={`Reproduzir ${item.title || `Video ${index + 1}`}`}>
              <img loading="lazy" src={thumbnail} alt={item.title || `Video ${index + 1}`} className="w-full h-full object-cover" />
            ...
            <iframe src={autoplayUrl || embedUrl} title={item.title || `Video ${index + 1}`} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
          )}
        </AspectRatio>
        {item.title && <div className="p-4"><p className="text-sm md:text-base font-medium text-foreground text-center">{item.title}</p></div>}
      </div>
    </CarouselItem>
  );
};
  ...
  return (
    <section className="w-full py-16 md:py-24 px-4 md:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <Carousel opts={{ align: "start" }} className="w-full max-w-7xl mx-auto mt-8 md:mt-12">
          <CarouselContent className="py-4">
            {data.items.map((item, index) => <VideoCard key={index} item={item} index={index} />)}
          </CarouselContent>
