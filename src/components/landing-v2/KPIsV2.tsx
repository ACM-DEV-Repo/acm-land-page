        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
          {enabledKpis.map((kpi, idx) => {
            ...
            <div key={idx} className="text-center glass-card p-10 md:p-14 hover:scale-[1.03] transition-all duration-300">
              <div className="flex justify-center mb-5"><IconBadgeV2 icon={IconComp} size="lg" /></div>
              <p className="text-5xl md:text-6xl lg:text-7xl font-bold text-[hsl(var(--ds-color-accent))] mb-4 md:mb-6">{kpi.value}</p>
              <p className="text-lg md:text-xl lg:text-2xl font-bold text-foreground mb-3">{kpi.label}</p>
              {kpi.description && <p className="text-sm md:text-base mt-3 text-muted-foreground leading-relaxed">{kpi.description}</p>}
            </div>
            );
          })}
        </div>
