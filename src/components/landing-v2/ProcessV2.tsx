...
type ProcessV2Props = { data: LPContent["process"]; lpKey: string; couponCode?: string };
          ...
          {data.steps?.map((step, index) => {
            ...
            <div key={index} className="glass-card p-8 md:p-10 hover:scale-[1.02] transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <IconComp size={24} className="text-[hsl(var(--ds-color-icon))] flex-shrink-0" />
                <h3 className="text-xl md:text-2xl font-bold text-foreground leading-tight">{step.title}</h3>
              </div>
        ...
        {(data.imageDesktop || data.imageMobile) && (
          <div className="w-full max-w-5xl mx-auto mt-12 md:mt-20 rounded-[var(--ds-radius)] aspect-video overflow-hidden glass-card">
            <picture>
              ...
              <img loading="lazy" src={data.imageDesktop || data.imageMobile} className="w-full h-full object-cover" alt="Processo" />
            </picture>
          </div>
        )}
